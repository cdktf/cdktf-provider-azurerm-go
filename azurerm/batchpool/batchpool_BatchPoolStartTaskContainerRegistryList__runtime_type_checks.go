...
func (j *jsiiProxy_BatchPoolStartTaskContainerRegistryList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*BatchPoolStartTaskContainerRegistry:
		val := val.(*[]*BatchPoolStartTaskContainerRegistry)
	...
	case []*BatchPoolStartTaskContainerRegistry:
		val_ := val.([]*BatchPoolStartTaskContainerRegistry)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*BatchPoolStartTaskContainerRegistry; received %#v (a %T)", val, val)
		}
	}
