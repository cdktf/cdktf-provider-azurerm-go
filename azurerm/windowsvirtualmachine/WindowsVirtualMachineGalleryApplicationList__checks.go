...
func (w *jsiiProxy_WindowsVirtualMachineGalleryApplicationList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_WindowsVirtualMachineGalleryApplicationList) validateSetInternalValueParameters(val interface{}) error {
	...
	case []*WindowsVirtualMachineGalleryApplication:
		val_ := val.([]*WindowsVirtualMachineGalleryApplication)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
