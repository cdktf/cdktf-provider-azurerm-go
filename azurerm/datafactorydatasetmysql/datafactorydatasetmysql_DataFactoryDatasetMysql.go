...
import (
	...
	"github.com/cdktf/cdktf-provider-azurerm-go/azurerm/v4/datafactorydatasetmysql/internal"
...
func NewDataFactoryDatasetMysql(scope constructs.Construct, id *string, config *DataFactoryDatasetMysqlConfig) DataFactoryDatasetMysql {
	...
	if err := validateNewDataFactoryDatasetMysqlParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func DataFactoryDatasetMysql_IsConstruct(x interface{}) *bool {
	...
	if err := validateDataFactoryDatasetMysql_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
...
// Experimental.
func DataFactoryDatasetMysql_IsTerraformElement(x interface{}) *bool {
	_init_.Initialize()

	if err := validateDataFactoryDatasetMysql_IsTerraformElementParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
...
// Experimental.
func DataFactoryDatasetMysql_IsTerraformResource(x interface{}) *bool {
	_init_.Initialize()

	if err := validateDataFactoryDatasetMysql_IsTerraformResourceParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
