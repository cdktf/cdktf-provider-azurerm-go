...
import (
	...
	"github.com/cdktf/cdktf-provider-azurerm-go/azurerm/v4/fluidrelayserver/internal"
...
func NewFluidRelayServer(scope constructs.Construct, id *string, config *FluidRelayServerConfig) FluidRelayServer {
	...
	if err := validateNewFluidRelayServerParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func FluidRelayServer_IsConstruct(x interface{}) *bool {
	...
	if err := validateFluidRelayServer_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
...
// Experimental.
func FluidRelayServer_IsTerraformElement(x interface{}) *bool {
	_init_.Initialize()

	if err := validateFluidRelayServer_IsTerraformElementParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
...
// Experimental.
func FluidRelayServer_IsTerraformResource(x interface{}) *bool {
	_init_.Initialize()

	if err := validateFluidRelayServer_IsTerraformResourceParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
