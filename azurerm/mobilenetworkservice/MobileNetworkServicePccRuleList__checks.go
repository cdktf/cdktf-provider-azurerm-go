...
func (j *jsiiProxy_MobileNetworkServicePccRuleList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktf.IResolvable:
		// ok
	case *[]*MobileNetworkServicePccRule:
		val := val.(*[]*MobileNetworkServicePccRule)
	...
	case []*MobileNetworkServicePccRule:
		val_ := val.([]*MobileNetworkServicePccRule)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*MobileNetworkServicePccRule; received %#v (a %T)", val, val)
		}
	}
