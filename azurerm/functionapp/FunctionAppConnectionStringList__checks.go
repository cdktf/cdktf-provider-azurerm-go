...
func (f *jsiiProxy_FunctionAppConnectionStringList) validateResolveParameters(context cdktf.IResolveContext) error {
	if context == nil {
...
func (j *jsiiProxy_FunctionAppConnectionStringList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktf.IResolvable:
		// ok
	case *[]*FunctionAppConnectionString:
		val := val.(*[]*FunctionAppConnectionString)
	...
	case []*FunctionAppConnectionString:
		val_ := val.([]*FunctionAppConnectionString)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*FunctionAppConnectionString; received %#v (a %T)", val, val)
		}
	}
