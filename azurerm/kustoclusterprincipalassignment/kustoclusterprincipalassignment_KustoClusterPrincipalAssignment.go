...
import (
	...
	"github.com/cdktf/cdktf-provider-azurerm-go/azurerm/v4/kustoclusterprincipalassignment/internal"
...
func NewKustoClusterPrincipalAssignment(scope constructs.Construct, id *string, config *KustoClusterPrincipalAssignmentConfig) KustoClusterPrincipalAssignment {
	...
	if err := validateNewKustoClusterPrincipalAssignmentParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func KustoClusterPrincipalAssignment_IsConstruct(x interface{}) *bool {
	...
	if err := validateKustoClusterPrincipalAssignment_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
...
// Experimental.
func KustoClusterPrincipalAssignment_IsTerraformElement(x interface{}) *bool {
	_init_.Initialize()

	if err := validateKustoClusterPrincipalAssignment_IsTerraformElementParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
...
// Experimental.
func KustoClusterPrincipalAssignment_IsTerraformResource(x interface{}) *bool {
	_init_.Initialize()

	if err := validateKustoClusterPrincipalAssignment_IsTerraformResourceParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
