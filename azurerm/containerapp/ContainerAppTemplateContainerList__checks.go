...
func (c *jsiiProxy_ContainerAppTemplateContainerList) validateResolveParameters(context cdktf.IResolveContext) error {
	if context == nil {
...
func (j *jsiiProxy_ContainerAppTemplateContainerList) validateSetInternalValueParameters(val interface{}) error {
	...
	case []*ContainerAppTemplateContainer:
		val_ := val.([]*ContainerAppTemplateContainer)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
